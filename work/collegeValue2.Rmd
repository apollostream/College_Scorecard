...
**IMPORTANT: This is an exploratory data analysis.**
It is not a commentary on the actual value represented by a college.
Future earnings aren't the sum total of the value of a college education.

### College Exploration Application

**See the [Best Colleges for You](https://thompsonml.shinyapps.io/BestCollegeApp/) web-based app to explore this same College Scorecard database.**

### Earnings Premium
We begin with this premise:

+   *The earnings permium for a college is the earnings in excess of the average (across all colleges) of the median annual earnings, measured at $6$ years after matriculation, plus key factors other than college reputation that might predict earnings.*

The earnings premium for college $i$, $\pi_i$, is given by the following formula:

\[
\pi_i \equiv e_i - (\bar{e} + f(A_i,\mathbf{D}_i,L_i)),
\]

where $e_i$ is the reported median annual earnings for students from college $i$ at 
$6$ years after matriculation;  $\bar{e} \equiv \frac{1}{N}\times\sum_{i=1}^N{e_i}$;
and $\sum_{i=1}^N{\pi_i}\equiv 0$.
...
Academic ability $A_i$ is measured by SAT. 
Academic discipline $\mathbf{D}_i$, by percentages of students receiving each type of degree $d_{ij}$.  
...
**Therefore, we intend that the earnings premium $\pi_i$ captures the impact of the reputation of college $i$ on the future earnings of its students.**
...
(1)   The expected annual earnings for college $i$, $e_i^*$ is gotten by adding the earnings premium $\pi_i$ to the overall mean (across all colleges) of annual earnings 6 years after matriculation $\bar{e}$, thus controlling the reported earnings $e_i$ for $A_i$, $\mathbf{D}_i$, and $L_i$.
...
```{r dataprep, echo = FALSE,warning=FALSE,message=FALSE,results='hide'}
...
fieldNames <-
  c(
    'UGDS','UGDS_WHITE','UGDS_BLACK','UGDS_HISP','UGDS_ASIAN','UGDS_NRA',
    "pell_ever",
    ...
    "C150_4_POOLED_SUPP",
    "C150_4_WHITE","C150_4_BLACK","C150_4_HISP","C150_4_ASIAN",'C150_4_NRA',
    "md_earn_wne_p6","md_earn_wne_p8","md_earn_wne_p10",
    "pct_white","pct_black","pct_asian","pct_hispanic","pct_ba","pct_grad_prof",
      "pct_born_us","md_faminc","median_hh_inc"
  )
...
disciplines2003 <- dbGetQuery( db, queryStringDscplns ) %>% 
  tbl_df() %>% 
  mutate_at( .funs=funs(as.numeric), .vars =vars( one_of(discNames) ) ) 
...
#    distance    == 'Not distance-education only',
  ...
  mutate_at(.funs=funs(ifelse(is.na(.),Treasury_md_earn_wne_p6,.)), .vars=vars(starts_with('Treasury_mn_earn_wne_')))
...
The Bayes factor $\operatorname{BF}$ is the ratio of the posterior-odds of the hypothesis (i.e., after receiving the evidence) to the prior-odds of the hypothesis (i.e., before seeing the evidence).  
...
We compute the Bayes factor, $\operatorname{BF}_{\log10}$ by applying $\log_{10}$ to the 
posterior-to-prior odds ratio.  The equations below show the calculation for  $\operatorname{BF}_{\log10}$
defining the feature corresponding to attribute $\text{Attribute }Y$ for each college of interest
$\text{College }X$.

\[
\begin{aligned}
&\operatorname{BF}_{\log10}( H = \text{College }X | E = \text{Attribute }Y ) \\
&= \log_{10}\Bigg[\frac{ \operatorname{Odds}( H = \text{College }X | E = \text{Attribute }Y )}{\operatorname{Odds}( H = \text{College }X ) }\Bigg]\\
&= \log_{10}\Bigg[ \frac{\operatorname{P}( E = \text{Attribute }Y | H = \text{College }X )}{\operatorname{P}( E = \text{Attribute }Y | H = \lnot (\text{College }X) )} \Bigg]\\
&\approx \log_{10}\Bigg[ \frac{ \operatorname{P}( E = \text{Attribute }Y | H = \text{College }X )}{\operatorname{P}( E = \text{Attribute }Y )} \Bigg]\\
\end{aligned}
\]

$E$ represents the *evidence* -- in this case, observing a college student with 
$\text{Attribute }Y$. $H$ represents the *hypothesis* -- in this case, the proposition that 
the student is enrolled at $\text{College }X$.

Above, the flip in the propositions on each side of the conditional ("|") when we go from $\operatorname{Odds}(H|E)$ to probabilities, $\operatorname{P}(E|H)$, occurs by applying Bayes' Rule.

The final approximation is a good one because there are 900 colleges in our working set; so the students attending $\text{College }X$ are a very small proportion of the entire American student population. Therefore, the probability of finding a student with $\text{Attribute }Y$ amongst students *not* at $\text{College }X$ is basically the same as finding such a student amongst the entire student population ($\text{College }X$ included).

#### Estimating the Probabilities

For some of the probabilities used in approximating the Bayes factors, the probabilities $\operatorname{P}( E = \text{Attribute }Y | H = \text{College }X )$ are simply the reported proportions in the database. 
...
#===================================================
  ...
  aid <- student %>% 
    ...
    group_by( unitID ) %>%
    do(
      (.) %>% mutate_at(.funs=funs(ifelse(is.na(.),(.)[!is.na(.)][[1]],.)),.vars=vars(matches("pell_ever|fsend")))
    ) %>%
    ungroup() %>%
    mutate(probSchool = UGDS/sum(UGDS),
  ...
  # Add interactions...
  studentBF %<>% 
    mutate_at(.funs=funs(10^(.)),.vars=vars(starts_with('BF')) ) %>% 
...
Now, we have the model's design matrix, `model_mat`. We use `cv.glmnet` to perform variable
selection to arrive at the final model for each college's earnings `outcome`.

```{r glmnet_cv,fig.height=8,fig.width=10}
...
Let's plot the colleges in 2-D space to see if we can find patterns. We use [*t-Distributed Stochastic Neighborhood Embeddings*](http://lvdmaaten.github.io/tsne/) (t-SNE) on the relevant predictors of the outcome variable to find a good 2-D representation in which we hope
to be able to identify clusters of similar colleges having similar earnings outcomes.
...
```
```{r tsne_glmnet,fig.height=10,fig.width=11}
set.seed( 2393 )
...
A t-SNE biplot can be created by overlaying the colleges in the 2-D t-SNE space on top of the
dimensions defined by the terms in a linear model between the outcome model's predictors and the t-SNE coordinates.

In this way, we can readily interpret which features (Bayes factors) define the similarity amongst clustered
colleges.

```{r biplot, warning=FALSE,message=FALSE,fig.height=10,fig.width=11}
...
See ["Bayesian Blocks in Two or More Dimensions: Image Segmentation and Cluster Analysis"](https://arxiv.org/abs/math/0111128) by J.D. Scargle, Nov. 11, 2001,
*Workshop on Bayesian Inference and Maximum Entropy Methods in Science and Engineering (MAXENT 2001)*,
held at Johns Hopkins University, Baltimore, MD USA on August 4-9, 2001.

See ["Voronoi Diagram and Delaunay Triangulation in R"](https://flowingdata.com/2016/04/12/voronoi-diagram-and-delaunay-triangulation-in-r/) 
by Nathan Yau, April 12, 2016.


```{r voronoi,eval=FALSE,echo=FALSE}
# Calculate tessellation and triangulation
vtess <- deldir( c(tsne_df$Y1), c(tsne_df$Y2) )

# Plot Voronoi tessellation.
plot(tsne_df %>% select(Y1,Y2), type="n", asp=1)
points(tsne_df %>% select(Y1,Y2), pch=20, col="red", cex=0.5)
plot(vtess, wlines="tess", wpoints="none", number=FALSE, add=TRUE, lty=1)

# # Delaunay triangulation
# plot(tsne_out$Y, type="n", asp=1)
# plot(vtess, wlines="triang", wpoints="none", number=FALSE, add=TRUE, lty=1)
# points(tsne_out$Y, pch=20, col="black", cex=0.5)

# Get the Voronoi tessalation summary and augment with density, N, and V for Beta function.

# Get the neighbors of every point -- as long as index of neighbor 
# is GREATER THAN THAT OF CURRENT POINT.
neighbors_tess <- function(df,ipt) {
  df %>% 
    filter(ind1==ipt | ind2 == ipt ) %$% 
    {setdiff(union(ind1,ind2),1:ipt)} %>%
    {list(nbrs=.,ipt=ipt, all_nbrs = sort(union(ind1,ind2)) )}
}
...
nt <- vtess$dirsgs %>% 
  ...
  { 
    lapply(
      seq_len( vtess$n.data ),
      neighbors_tess,
      df=.
    ) 
  }

min_dir_area <- min( vtess$summary$dir.area )
vor_tess <- vtess$summary %>% 
  as_tibble() %>% 
  mutate(
    density = 1.0/dir.area,
    N = 1,
    V = dir.area/min_dir_area,
    Phi = beta( N+1, V-N+1 ),
    peers = seq_len(nrow(.))
  )


# Compute Bayesian Blocks by coalescence of tessalations
iter_max <- choose( n = vtess$n.data, k = 2 )
max_merge_limit <- 30
...
while( !done ){
  merge_factor <- sapply(
    nt,
    function(nbr_set){
      if(length(nbr_set$nbrs)>0){
        p_mrg <- sapply(
          nbr_set$nbrs,
          function(inbr){
            N_cmb <- vor_tess$N[[nbr_set$ipt]] + vor_tess$N[[inbr]]
            V_cmb <- vor_tess$V[[nbr_set$ipt]] + vor_tess$V[[inbr]] 
            return(
              c(
                p  = beta(N_cmb+1,V_cmb-N_cmb+1)/(vor_tess$Phi[[nbr_set$ipt]]*vor_tess$Phi[[inbr]]),
                id = inbr
              )
            )
          }
        )
        #i_max <- which.max(p_mrg['p',])
        i_max <- which(p_mrg['p',]>1) %>% {(.)[which.min( p_mrg['p',.] )]}
        if(length(i_max)==0){
          c( id1 = nbr_set$ipt, i_max = 0, p_mrg = 0 )
        } else {
          c( id1 = nbr_set$ipt, i_max = p_mrg['id',i_max], p_mrg = p_mrg['p',i_max] )
        }
      } else {
        c( id1 = nbr_set$ipt, i_max = 0, p_mrg = 0 )
      }
  ...
  #i_max <- which.max(merge_factor['p_mrg.p',])
  i_max <- which(merge_factor['p_mrg.p',]>1) %>% {(.)[which.min( merge_factor['p_mrg.p',.] )]}
  max_merge <- merge_factor['p_mrg.p',i_max]
  done <- length(max_merge)==0 || max_merge > max_merge_limit 
  
  if(!done){
    #do_merge
    id1 <- merge_factor['id1',i_max]
    id2 <- merge_factor['i_max.id',i_max]
    ...
    nt[[id1]]$all_nbrs %<>%
      union( nt[[id2]]$all_nbrs )
    nt[[id2]]$all_nbrs <- nt[[id1]]$all_nbrs
    
    for( i in nt[[id2]]$all_nbrs[which(nt[[id2]]$all_nbrs>id1 & nt[[id2]]$all_nbrs<id2)] ){
      nt[[i]]$nbrs %<>% setdiff(id2) %>% union(id1)
      nt[[i]]$all_nbrs %<>% union(id1)
      nt[[id1]]$all_nbrs %<>% union(i)
    }
    
    vor_tess$N[[id1]] %<>% add( vor_tess$N[[id2]] )
    vor_tess$V[[id1]] %<>% add( vor_tess$V[[id2]] )
    vor_tess$Phi[[id1]] <- beta(vor_tess$N[[id1]]+1,vor_tess$V[[id1]]-vor_tess$N[[id1]]+1)
    vor_tess$peers[[id1]] <- list(
      union( 
        unlist( vor_tess$peers[[id1]] ),
        unlist( vor_tess$peers[[id2]] )
      )
    )
    #print(vor_tess[id1,])
  }
  iter <- iter + 1
  print(c(iter=iter,max_merge))
  done <- done || iter >= iter_max
}
...
points(tsne_df %>% select(Y1,Y2), pch=20, col="red", cex=0.5)
plot(vtess, wlines="tess", wpoints="none", number=FALSE, add=TRUE, lty=1)
prs <- vor_tess %>% filter( N >= n_min ) %$% peers; 
for(j in seq_along(prs)) {
  points(
    tsne_df %>% select(Y1,Y2) %>% slice(unlist(prs[[j]])), 
    pch = 20, 
    col = j, 
    cex = 1
  )
...
```{r hclustering, warning=FALSE,message=FALSE,fig.height=10,fig.width=11}
      ...
      geom_point( 
        data = tsne_mat_hc %>%
          ...
          mutate( 
            College = names(cluster_id), 
            cluster = factor( (cluster_id %% 7) + 1 ),
            cluster_shape = factor( (cluster_id %% 6) + 1 )
          ), 
...
```{r pred_eprem,warning=FALSE,message=FALSE,fig.width=10,fig.height=8}
...
# abbreviate_college <- function( df, college_col ){
#   college_col <- enquo( college_col )
#   cname       <- quo_name( college_col )
#   nms <- df %>%
#     dplyr::select(!!college_col) %>%
...
makeStudentValue <- function( 
  ...
  student_value <- studentBF %>% select(-College,-outcome,-matches('Treasury|pell|Year2003')) %>%
    ...
    mutate(
      ...
      vnorm   = (Earnings_Adjusted/Living_Expenses - 1.0) / (Earnings_Adjusted[[1]]/Living_Expenses[[1]] - 1.0), # Earnings normalized by that of the highest utility school.
