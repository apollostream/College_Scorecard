...
We compute the Bayes factor, $\operatorname{BF}_{\log10}$, i.e. applying $\log_{10}$ to the 
posterior to prior odds ratio, using the following 
for the feature corresponding to attribute $\text{Attribute }Y$ for each college of interest
$\text{College }X$, where $E$ represents the *evidence* -- in this case, observing a college student with 
$\text{Attribute }Y$ -- and $H$ represents the *hypothesis* -- in this case, the proposition that 
the student is enrolled at $\text{College }X$ (we use log-base-10 on the Bayes factor.):

\[
\begin{aligned}
&\operatorname{BF}_{\log10}( H = \text{College }X | E = \text{Attribute }Y ) \\
&= \log_{10}\frac{ \operatorname{Odds}( H = \text{College }X | E = \text{Attribute }Y )}{\operatorname{Odds}( H = \text{College_X} ) )}\\
&= \log_{10} \frac{\operatorname{P}( E = \text{Attribute }Y | H = \text{College }X )}{\operatorname{P}( E = \text{Attribute }Y | H = \lnot (\text{College }X) )}\\
&\sim \log_{10} \frac{ \operatorname{P}( E = \text{Attribute }Y | H = \text{College }X )}{\operatorname{P}( E = \text{Attribute }Y )}\\
\end{aligned}
\]
...
### Visualization of College (t-SNE Plot)

Let's plot the colleges in 2-D space to see if we can find patterns.
...
```
```{r tsne_glmnet,fig.height=8,fig.width=10}
set.seed( 2393 )
...
#tsne_glmnet_coef <- tsne_glmnet_cv %$% coef(.,s=exp(mean(log(c(lambda.1se,lambda.min)))))
...
categories <- { 
  ...
  sapply(function(x,q){ length(q) - sum(x>q) + 1 },q=quantile(.,c(0.1,0.25,0.75,0.9))) %>%
  factor()

tsne_df <- tsne_out$Y %>% 
  as_tibble() %>% 
  setNames(c("Y1","Y2")) %>% 
  mutate( College = college_names, category = categories, Y2 = -Y2 ) %>% 
  dplyr::select( College, category, everything() ) %>% 
  mutate_at(funs(1.5*scale(.)),.vars=vars(Y1,Y2))
...
tsne_coef_df %>% 
  mutate( mag = sqrt(Y1^2 + Y2^2) ) %>%
  {
    ggplot(., aes( x = Y1, y = Y2 ) ) +
      ...
      geom_point( data=tsne_df, aes(x=Y1,y=Y2), alpha=0.3) +
      ggtitle( "t-SNE Biplot" , subtitle = "(black = item, red = dimension)") +
      theme( text = element_text( face = 'bold' ) )
  } %>%
  print()
