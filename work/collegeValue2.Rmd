...
# system.time( 
#   { 
    tsne_out <- Rtsne( model_mat[model_mat[,'Year2003']==0,!grepl('College',df_coef$Coefficient[-1])], perplexity = 10 ) 
#   } 
# )

college_names <- glmdata %>% filter(Year2003 == 0 ) %$% College_ID %>%
{ gsub('^[0-9_]+','',. ) } %>%
{ gsub('California','Cal',. ) } %>%
...
categories <- glmdata %>% filter(Year2003 == 0 ) %$% BF_SAT_gt1200le1400 %>% 
...
tsne_out$Y %>% 
  as_tibble() %>% 
  setNames(c("Y1","Y2")) %>% 
  mutate( College = college_names, category = categories ) %>% 
  dplyr::select( College, category, everything() ) %>%
  { 
    ggplot(.,aes(x=Y1,y=Y2,color=category)) + 
      geom_point(size=2) + 
      geom_text(aes(label=College),size=2,check_overlap = TRUE) +
...
tsne_glmnet_cv <- cv.glmnet(
  model_mat[model_mat[,'Year2003']==0,grep('College_ID',df_coef$Coefficient[-1],value=TRUE,invert = TRUE)], #glmdata %>% select(-outcome) %>% as.matrix(),
  tsne_out$Y,
  family='mgaussian',
...
tsne_glmnet_coef <- tsne_glmnet_cv %$%
  coef(.,s=exp(mean(log(c(lambda.1se,lambda.min)))))
tsne_glmnet_coef$y1 %>%
{ (.)[abs((.)[,1])>0,1] } %>%
